...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const allowed = checkRateLimit(ip, 'invoice-description');
  ...
  try {
    const completion = await groqTools.chat.completions.create({
      ...
      messages: [{ 
        role: 'user', 
        content: `Write professional invoice line item descriptions for ${sService} provided to ${sClientName} for ${sHours} hours. Write 3 professional line items. 150 words maximum.` 
      }],
      max_tokens: 500,
    });
    ...
    console.error('Invoice description generation error:', error);
