...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const allowed = checkRateLimit(ip, 'social-media-report');
  ...
  try {
    const completion = await groqTools.chat.completions.create({
      ...
      messages: [{ 
        role: 'user', 
        content: `Write a social media performance report for ${sClientName} on ${sPlatforms}. Metrics: ${sMetrics}. Include: Performance Overview, Platform Breakdown, Key Wins, Next Month Strategy. 300 words maximum.` 
      }],
      max_tokens: 500,
    });
